...
命令、服务商、路由和发布的全部文档都在这里。
[README](../README.zh-CN.md) 负责安装、配置和日常使用，下面的内容留给需要细节的人。
...
Windows x64、Windows ARM64、macOS Apple 芯片和 macOS Intel 四个安装包都在
[v0.1.20 发行页](https://github.com/konbakuyomu/smartsearch/releases/tag/v0.1.20)，旁边附 `SHA256SUMS.txt`。
原生 App 提供概览、服务商、搜索与研究、活动、AI 接入和设置页面，自带运行环境，无需另装 Python 或 Node。
安装包未签名，macOS 未公证；目前只有 Windows x64 完成了实机验收。详见[桌面端使用说明](desktop.md)。
...
当前稳定版是 `0.1.20`，改动写在
[v0.1.20 发行页](https://github.com/konbakuyomu/smartsearch/releases/tag/v0.1.20)，
更早的版本说明放在 `.github/releases/` 下。
