# smart-search Full Reference

The complete command, provider, routing, and release documentation. The
[README](../README.md) covers install, configuration, and daily use;
everything below is for people who need the details.
...
Installers for Windows x64, Windows ARM64, macOS Apple silicon and macOS Intel
are on the [v0.1.20 release page](https://github.com/konbakuyomu/smartsearch/releases/tag/v0.1.20),
with a `SHA256SUMS.txt` next to them. The native app ships its own runtime and
provides overview, providers, search/research, activity, AI-integration and
settings pages. No separate Python or Node installation is required for the app.
The installers are unsigned and the macOS builds are not notarized; Windows x64
is the only one validated on real hardware so far. See the [desktop guide](desktop.md).
...
The current stable version is `0.1.20`. Read the changes on the
[v0.1.20 release page](https://github.com/konbakuyomu/smartsearch/releases/tag/v0.1.20);
earlier notes live under `.github/releases/`.
