...
- `smart-search search QUERY [--platform NAME] [--model ID] [--extra-sources N] [--validation fast|balanced|strict] [--fallback auto|off] [--providers auto|CSV] [--stream|--no-stream] [--timeout SECONDS] [--format json|markdown|content] [--output PATH]` (`--timeout` overrides the persisted `SMART_SEARCH_TIMEOUT_SECONDS` total budget)
...
- `smart-search providers status [--format json|markdown|content] [--output PATH]`
- `smart-search providers reset [PROVIDER...] [--format json|markdown|content] [--output PATH]`
- `smart-search regression`
- `smart-search smoke [--mode mock|live] [--mock] [--live] [--format json|markdown|content] [--output PATH]`

## Aliases

Top-level aliases normalize to the same service behavior as their full command: `search`/`s`, `route`/`rt`, `fetch`/`f`, `map`/`m`, `exa-search`/`exa`/`x`, `exa-similar`/`xs`, `zhipu-search`/`z`/`zp`, `zhipu-mcp-search`/`zmcp-search`, `zhipu-mcp-reader`/`zmcp-reader`, `zhipu-mcp-search-doc`/`zmcp-doc`, `zhipu-mcp-repo-structure`/`zmcp-tree`, `zhipu-mcp-read-file`/`zmcp-file`, `anysearch-domains`/`as-domains`, `anysearch-search`/`as-search`/`as`, `anysearch-extract`/`as-extract`, `anysearch-batch`/`as-batch`, `sciverse-catalog`/`sv-catalog`, `sciverse-search`/`sv-search`/`sv`, `sciverse-semantic`/`sv-semantic`, `sciverse-read`/`sv-read`, `sciverse-relations`/`sv-relations`, `context7-library`/`c7`/`ctx7`, `context7-docs`/`c7d`/`c7docs`/`ctx7-docs`, `deep`/`dr`, `research`/`rs`, `route-calibrate`/`route-cal`/`rcal`, `doctor`/`d`, `diagnose`/`diag`, `setup`/`init`, `config`/`cfg`, `model`/`mdl`, `smoke`/`sm`, `providers`/`prov`, and `regression`/`reg`.

Nested aliases: `config path`/`cfg p`, `config list`/`cfg ls`/`cfg l`, `config set`/`cfg s`, `config unset`/`cfg rm`/`cfg u`, `model current`/`mdl cur`/`mdl c`, `model set`/`mdl s`, `providers status`/`prov st`/`prov ls`, and `providers reset`/`providers clear`.
...
- Fetch output includes `ok`, `url`, `provider`, `content`, `provider_attempts`, `provider_notices`, `fallback_used`, and `elapsed_ms`.
...
- Research executor output includes `ok`, `mode=deep_research_execution`, `query_mode=research`, `question`, `budget`, `research_plan`, `routing_decision`, `stage_results`, `discovery_sources`, `final_answer`, `content`, `citations`, `evidence_items`, `gap_check`, `provider_attempts`, `provider_notices`, `providers_used`, `fallback_used`, `degraded`, `route_policy_version`, `evidence_dir`, `minimum_profile_ok`, `capability_status`, and `elapsed_ms`.
- Diagnostic output masks keys and reports config paths, Windows legacy config metadata, provider timeout values, `capability_status`, `minimum_profile_ok`, `intent_router_status`, `provider_health`, `main_search_connection_tests`, and provider connectivity checks. A successful `doctor` probe also clears that provider's failure cooldown. OpenAI-compatible health must use the endpoint selected by `OPENAI_COMPATIBLE_API_MODE`; `/models` is supplementary metadata.
- Provider health output includes `ok`, `enabled`, `cooldown_seconds`, `failure_threshold`, `store_path`, `providers`, and `cooldown_providers`; each provider entry carries `provider`, `configured`, `state` (`closed` or `cooldown`), `consecutive_failures`, `error_type`, `error`, `cooldown_remaining_seconds`, and `hard_failure`. `providers reset` output includes `ok`, `cleared`, and `known_providers`, and returns `parameter_error` for an unknown provider id.
- `provider_notices` is one deduplicated entry per degraded optional provider with `provider`, `capability`, `status` (`failed` or `cooldown`), `error_type`, `error`, `cooldown_remaining_seconds`, and `hint`. Markdown output prints a steady `cooldown` notice only when the command itself failed; JSON always carries it.
