...
- `OPENAI_COMPATIBLE_API_MODE` selects `chat-completions` or `responses` for the
  compatible route and defaults to `chat-completions`. Any other value is a
  parameter error.
...
| Invalid `OPENAI_COMPATIBLE_API_MODE` | Return `error_type: "parameter_error"` |
